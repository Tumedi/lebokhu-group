// ============================================================
// LeBoKhu Group — Edge Function: send-provider-approved
// ------------------------------------------------------------
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const FROM = Deno.env.get("PROVIDER_FROM") ?? "LeBoKhu Group <onboarding@resend.dev>";
    ...
    const html = `
      <div style="font-family:Arial,Helvetica,sans-serif;max-width:560px;margin:0 auto;color:#0B2038">
        ...
        <div style="border:1px solid #e6ebf0;border-top:none;border-radius:0 0 12px 12px;padding:24px">
          ...
          <strong>approved</strong> and is now live in the LeBoKhu Group directory.</p>
    ...
    const text =
      `Hi ${name},\n\n` +
      `Great news! Your service listing has been approved and is now live in the LeBoKhu Group directory.\n\n` +
    ...
    const resendRes = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject: "Your service listing is approved — LeBoKhu Group",
        html, text,
      }),
    });
