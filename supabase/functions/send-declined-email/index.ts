// ============================================================
// LeKhuBo Connect — Supabase Edge Function: send-declined-email
// ------------------------------------------------------------
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const html = `
      <div style="font-family:Arial,Helvetica,sans-serif;max-width:560px;margin:0 auto;color:#0B2038">
        ...
        <div style="border:1px solid #e6ebf0;border-top:none;border-radius:0 0 12px 12px;padding:24px">
          <p>Hi ${esc(name)},</p>
          <p>Thank you for submitting your job post <strong>${esc(title)}</strong>${company ? " for " + esc(company) : ""} with LeKhuBo Connect.</p>
    ...
    const text =
      `Hi ${name},\n\n` +
      `Thank you for submitting your job post "${title}"${company ? " for " + company : ""} with LeKhuBo Connect.\n\n` +
